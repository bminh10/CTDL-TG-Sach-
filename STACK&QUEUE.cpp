...
//first: thoi gian bat dau, second: khoang thoi gian thuc hien
...
int main() {
	...
	//chi dinh do uu tien 
	/*vd: Cho danh sach cong viec co thoi gian bat dau va khoang thoi gian thuc hien tung cong viec. 
	Hay lap lich thuc hien voi cac cong viec sao cho cong viec co thoi gian bat dau som nhat chay truoc,
	neu 2 cong viec co cung thoi gian bat dau thi cong viec co thoi gian thuc hien ngan hon se thuc hien truoc.
	*/
	auto compare = [](auto e, auto f) {
		if (e.first == f.first) {
			return e.second > f.second;
		}
		return e.first > f.first;
		};
	priority_queue<Task, vector<Task>, decltype(compare)>e;
	...
	e.push({ 4,2 });
	e.push({ 3,2 });
	cout << "Thu tu thuc hien cong viec: " << endl;
	while (!e.empty()) {
		Task t = e.top();
